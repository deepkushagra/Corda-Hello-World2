...
import com.helloworld.flow.IOUFlow
...
// This API is accessible from /api/helloworld. All paths specified below are relative to it.
@Path("helloworld")
class ExampleApi(private val rpcOps: CordaRPCOps) {
     ...
     * see it when calling /api/helloworld/ious on their respective nodes.
     *
    ...
    fun createIOU(@QueryParam("iouValue") iouValue: Int, @QueryParam("partyName") partyName: CordaX500Name?): Response {
        ...
        return try {
            val signedTx = rpcOps.startTrackedFlow(::IOUFlow, iouValue, otherParty).returnValue.getOrThrow()
    ...
    fun myious(): Response {
        ...
        val results = builder {
                var partyType = IOUSchemaV.PersistentIOUSchema::lenderName.equal(rpcOps.nodeInfo().legalIdentities.first().name.toString())
